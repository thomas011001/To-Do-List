const storage = (() => {
  if (!localStorage.getItem("default")) {
    localStorage.setItem(
      "default",
      JSON.stringify({ description: "This's the default project", tasks: [] })
    );
  }
  const addProject = (project) => {
    if (localStorage.getItem(project.title)) {
      ...
      localStorage.setItem(
        project.title,
        JSON.stringify({ description: project.description, tasks: [] })
      );
    }
  };
  const getProject = (title) =>
    Object.assign({ title }, JSON.parse(localStorage.getItem(title)));
  ...
  const editProject = (title, newProject) =>
